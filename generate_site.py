...
Usage
-----
    pip install firebase-admin jinja2
    export FIREBASE_SERVICE_ACCOUNT='<contents of your service account json>'
    python generate_site.py

In GitHub Actions, store the service account JSON as a repository secret
(e.g. FIREBASE_SERVICE_ACCOUNT) and export it as an env var before running
this script. Locally, you can instead drop a `serviceAccountKey.json` file
next to this script (already gitignored) and it will be picked up
automatically.
...
BASE_URL = os.environ.get("BASE_URL", "https://vinoelite.com").rstrip("/")
OUTPUT_DIR = "public"
...
try:
    if not firebase_admin._apps:
        ...
        if sa_env:
            ...
            print("ERROR: no credentials found. Set FIREBASE_SERVICE_ACCOUNT "
                  "or place a serviceAccountKey.json file next to this script.")
            sys.exit(1)
...
jinja.globals.update(BASE_URL=BASE_URL, SITE_NAME=SITE_NAME)
