...
#
# BASE_PATH supports GitHub Pages "project sites" that live under a
# subpath, e.g. https://username.github.io/Wine/ instead of a custom
# domain at the root. It's derived automatically from --site-url: pass
# --site-url https://username.github.io/Wine and every internal link,
# the stylesheet path, the sitemap and the root/404 redirects all get the
# /Wine prefix automatically. Pass a root URL (custom domain, or
# https://username.github.io/) and BASE_PATH is simply empty.
# ---------------------------------------------------------------------------

BASE_PATH = ""  # set once at the start of build() from --site-url's path


def url_for(page: str, lang: str) -> str:
    filename = "index.html" if page == "index" else f"{page}.html"
    return f"{BASE_PATH}/{lang}/{filename}"


def url_for_category_slug(category_slug: str, lang: str) -> str:
    return f"{BASE_PATH}/{lang}/{PRODUCTS_PAGE}/{category_slug}.html"


def url_for_product_dict(p: dict, lang: str) -> str:
    return f"{BASE_PATH}/{lang}/{PRODUCTS_PAGE}/{p['categorySlug']}/{p['slug']}.html"
...
def schema_website(site_url: str) -> dict:
    return {
        ...
        "name": "VinoElite",
        "url": site_url,
        "logo": f"{site_url}{BASE_PATH}/assets/images/logo.png",
    }
...
def build(out_dir: Path, site_url: str, service_account: str | None, skip_firestore: bool, fixture: str | None = None):
    ...
    BASE_PATH = parsed.path.rstrip("/")  # e.g. "/Wine", or "" for a root/custom domain
    if BASE_PATH:
        print(f"  detected subpath deployment: BASE_PATH={BASE_PATH!r} (from --site-url {site_url!r})")
    ...
    for lang in LANGUAGES:
        ...
        common_ctx = dict(
            ...
            site_url=site_origin, og_locale=OG_LOCALES.get(lang, "en_US"),
            asset_path=f"{BASE_PATH}/assets/", base_path=BASE_PATH, year=year,
        )
        ...
        html = env.get_template("tpl_index.html").render(
            **common_ctx, page_url=page_url,
            ...
            schema_json=sj(schema_website(site_origin)),
        )
        ...
        breadcrumbs = sj(schema_breadcrumbs([(t["nav_home"], url_for("index", lang)), (t["nav_catalog"], page_url)], site_origin))
        html = env.get_template("tpl_catalog.html").render(
            **common_ctx, page_url=page_url,
            ...
            schema_json=sj(schema_itemlist(products, lang, site_origin)),
        ...
        for cat in categories_sorted:
            ...
            html = env.get_template("tpl_category.html").render(
                **common_ctx, page_url=page_url,
                ...
                schema_json=sj(schema_itemlist(cat_products, lang, site_origin)),
                breadcrumb_json=breadcrumbs,
            )
            write(out_dir, page_url, html)
        ...
        for p in products:
            ...
            html = env.get_template("tpl_product.html").render(
                **common_ctx, page_url=page_url, alt_urls=alt_urls,
                ...
                schema_json=sj(schema_product(p, lang, site_origin, page_url)),
                breadcrumb_json=breadcrumbs,
            )
            write(out_dir, page_url, html)
    ...
    html = env.get_template("tpl_404.html").render(
        ...
        site_url=site_origin, asset_path=f"{BASE_PATH}/assets/", year=year,
    ...
    if TOOLS_DIR.exists():
        shutil.copytree(TOOLS_DIR, out_dir / "tools")
        print("  copied tools/ (seed_firestore.html etc.)")

    write_root_redirect(out_dir)
    write_sitemap(out_dir, sitemap_entries, site_origin)


def write(out_dir: Path, url_path: str, html: str):
    # url_path already has BASE_PATH baked in (for hrefs); strip it back off
    # here because BASE_PATH is added by GitHub Pages itself (from the repo
    # name) — our docs/ folder must stay flat, e.g. docs/en/index.html, not
    # docs/Wine/en/index.html.
    relative = url_path
    if BASE_PATH and relative.startswith(BASE_PATH):
        relative = relative[len(BASE_PATH):]
    out_path = out_dir / relative.lstrip("/")
...
(function() {{
  ...
  window.location.replace("{BASE_PATH}/" + lang + "/index.html");
}})();
</script>
</head>
<body><p>Redirecting to <a href="{BASE_PATH}/en/index.html">VinoElite</a>...</p></body>
</html>
"""
