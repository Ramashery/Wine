...
// Site "sub-folder" the page is served from, e.g. "/Wine" when the site
// lives at https://user.github.io/Wine/ instead of a domain root. Every
// template writes it into <body data-base-path="..."> at build time
// (generate_site.py derives it from the --site-url you pass it), and every
// internal link/redirect in this file is prefixed with it so the site
// works unmodified whether it's hosted at a domain root or a subpath.
...
function createProductCardHTML(prod) { 
            ...
            const productUrl = `${BASE_PATH}/${categorySlug}/${prod.slug}`; 
            ...
            const slidesHTML = imageUrls.map((url, index) => `<div class="slideshow-item"><img src="${url}" alt="${prod.name} view ${index + 1}" loading="lazy" style="object-fit: contain;"></div>`).join(''); 
            ...
            return `<div class="product-card animate-on-scroll" data-url="${productUrl}">
                        <div class="slideshow-container">
                            ...
                            <button class="add-to-cart-btn" data-product-id="${prod.id}" aria-label="Add to cart">
...
function updateUIForAuthState(user) { if (user) { const userName = user.displayName || user.email.split('@')[0]; desktopAuthBtn.href = `${BASE_PATH}/profile.html`; desktopAuthBtn.innerHTML = `<i class="fas fa-user-check"></i> <span class="auth-text">${userName}</span>`; desktopAuthBtn.title = "My Account"; desktopAuthBtn.removeEventListener('click', openLoginModal); mobileAuthBtn.href = `${BASE_PATH}/profile.html`; mobileAuthBtn.querySelector('span').textContent = userName; mobileAuthBtn.removeEventListener('click', openLoginModal); } else { desktopAuthBtn.href = "#"; desktopAuthBtn.innerHTML = `<i class="far fa-user"></i> <span class="auth-text">Sign In</span>`; desktopAuthBtn.title = "Sign In"; desktopAuthBtn.addEventListener('click', openLoginModal); mobileAuthBtn.href = "#"; mobileAuthBtn.querySelector('span').textContent = 'Sign In / Register'; mobileAuthBtn.addEventListener('click', openLoginModal); } }

function openLoginModalForGuests(e) { e.preventDefault(); openLoginModal(e); }

function setupGuestInteractions() { const user = auth.currentUser; const wishlistLinks = document.querySelectorAll(`a[href="${BASE_PATH}/profile.html#wishlist"]`); wishlistLinks.forEach(link => { link.removeEventListener('click', openLoginModalForGuests); if (!user) { link.addEventListener('click', openLoginModalForGuests); } }); }
...
function initCommonUI() {
    ...
    if (desktopSearchBtn && searchModal) {
        ...
        desktopSearchForm.addEventListener('submit', (e) => {
            ...
            if (q) window.location.href = `${BASE_PATH}/catalog.html?search=${encodeURIComponent(q)}`;
        });
    }
    ...
    if (footer) {
        ...
        footer.addEventListener('click', () => {
            ...
            else if (clickCount === 2) { clearTimeout(clickTimer); clickCount = 0; window.location.href = `${BASE_PATH}/admin.html`; }
        });
    }
...
function initIndexPage() {
    ...
    function renderCategoryCarousels(products) {
        ...
        CATEGORIES_TO_DISPLAY.forEach(categoryName => {
            ...
            const categoryUrl = `${BASE_PATH}/${categorySlug}`;
...
function initCatalogPage() {
            ...
            productsToRender.forEach((prod, index) => {
                ...
                const slidesHTML = imageUrls.map((url, i) => 
                    `<div class="slideshow-item ${i===0?'active':''}">
                        <img src="${url}" alt="${prod.name} view ${i + 1}" loading="${loadingType}" width="300" height="400" style="aspect-ratio: 3/4; object-fit: contain;">
                     </div>`
                ).join('');
        ...
        function renderBreadcrumbs() {
            let html = `<a href="${BASE_PATH}/index.html">Home</a> / <a href="${BASE_PATH}/catalog.html">Catalog</a>`;
            let tempFilters = {};
            ...
            if (state.filters.category.length > 0) { 
                ...
                if (state.filters.category.length === 1) {
                    href = `${BASE_PATH}/${Object.keys(CATEGORY_SLUG_MAP).find(k => CATEGORY_SLUG_MAP[k] === state.filters.category[0]) || generateSlug(state.filters.category[0])}`;
                } else {
                    tempFilters.category = state.filters.category.join(','); 
                    href = `${BASE_PATH}/catalog.html?${new URLSearchParams(tempFilters)}`;
                }
            ...
            if (countriesToShow.size > 0) { const value = [...countriesToShow].sort().join(', '); tempFilters.country = state.filters.country.join(','); html += ` / <a href="${BASE_PATH}/catalog.html?${new URLSearchParams(tempFilters)}">${value}</a>`; }
            if (regionsToShow.size > 0) { const value = [...regionsToShow].sort().join(', '); if (state.filters.region.length > 0) tempFilters.region = state.filters.region.join(','); html += ` / <a href="${BASE_PATH}/catalog.html?${new URLSearchParams(tempFilters)}">${value}</a>`; }
            if (appellationsToShow.size > 0) { const value = [...appellationsToShow].sort().join(', '); if (state.filters.appellation.length > 0) tempFilters.appellation = state.filters.appellation.join(','); html += ` / <a href="${BASE_PATH}/catalog.html?${new URLSearchParams(tempFilters)}">${value}</a>`; }
        ...
        function parseUrlParams() {
            ...
            // Strip the site's base path (e.g. "/Wine") before reading the
            // category slug out of the URL, so pretty category URLs still
            // resolve correctly when the site is hosted under a sub-folder.
        ...
        function updateURL() {
            ...
            let newPath = `${BASE_PATH}/catalog.html`;
...
function renderProduct(product) { 
        ...
        const params = new URLSearchParams(); 
        let breadcrumbHTML = `<a href="${BASE_PATH}/index.html">Home</a> / <a href="${BASE_PATH}/catalog.html">Catalog</a>`; 
        ...
        if (product.country) { params.set('country', product.country); breadcrumbHTML += ` / <a href="${BASE_PATH}/catalog.html?${params.toString()}">${product.country}</a>`; } 
        if (product.region) { params.set('region', product.region); breadcrumbHTML += ` / <a href="${BASE_PATH}/catalog.html?${params.toString()}">${product.region}</a>`; } 
        if (product.appellation) { params.set('appellation', product.appellation); breadcrumbHTML += ` / <a href="${BASE_PATH}/catalog.html?${params.toString()}">${product.appellation}</a>`; } 
        ...
        mainGalleryContainer.innerHTML = imageUrls.map((url, i) => 
            `<div class="slideshow-item ${i===0?'active':''}">
                <img src="${url}" alt="${product.name} view ${i + 1}" loading="${i===0?'eager':'lazy'}" width="600" height="800" style="aspect-ratio: 3/4; object-fit: contain;">
             </div>`
        ...
        // --- Primary facts: Winery / Type / Color ---
        const primaryFacts = document.getElementById('pd-facts-primary');
        let primaryHTML = '';
        if (product.brand) primaryHTML += `<div class="pd-fact"><strong>Winery:</strong> <a href="${BASE_PATH}/catalog.html?brand=${encodeURIComponent(product.brand)}">${product.brand}</a></div>`;
        if (product.sweetness) primaryHTML += `<div class="pd-fact"><strong>Type:</strong> ${product.sweetness}</div>`;
        if (product.color) primaryHTML += `<div class="pd-fact"><strong>Color:</strong> <a href="${BASE_PATH}/catalog.html?color=${encodeURIComponent(product.color)}">${product.color}</a></div>`;
        primaryFacts.innerHTML = primaryHTML;

        // --- Secondary facts: Country / Region / Grape Variety / Vintage ---
        const secondaryFacts = document.getElementById('pd-facts-secondary');
        let secondaryHTML = '';
        if (product.color) { /* already shown above; kept out of secondary to match reference order */ }
        if (product.country) secondaryHTML += `<div class="pd-fact"><strong>Country:</strong> <a href="${BASE_PATH}/catalog.html?country=${encodeURIComponent(product.country)}">${product.country}</a></div>`;
        if (product.region) secondaryHTML += `<div class="pd-fact"><strong>Region:</strong> <a href="${BASE_PATH}/catalog.html?region=${encodeURIComponent(product.region)}">${product.region}</a></div>`;
        if (product.grapeVarieties) secondaryHTML += `<div class="pd-fact"><strong>Grape Variety:</strong> <a href="${BASE_PATH}/catalog.html?grape=${encodeURIComponent(product.grapeVarieties)}">${product.grapeVarieties}</a></div>`;
        if (product.year) secondaryHTML += `<div class="pd-fact"><strong>Vintage:</strong> ${product.year}</div>`;
        secondaryFacts.innerHTML = secondaryHTML;

        document.getElementById('product-pricing').innerHTML = `$${product.price.toFixed(2)}` + (product.oldPrice ? `<span class="old-price">$${product.oldPrice.toFixed(2)}</span>` : ''); 
        ...
        // Wishlist button (in the actions row, separate from the small icon on the image)
        const pdWishlistBtn = document.getElementById('pd-wishlist-btn');
        pdWishlistBtn.dataset.productId = product.id;

        // --- Aroma (description) + extra details ---
        const descEl = document.getElementById('product-description');
        descEl.innerHTML = product.description ? `<strong>Aroma:</strong> ${product.description}` : '';

        const extraDetails = document.getElementById('pd-extra-details');
        let extraHTML = '';
        if (product.tastingNotes) {
            product.tastingNotes.split('\n').forEach(line => {
                if (line.trim() === '') return;
                const [key, ...value] = line.split(':');
                extraHTML += `<p class="pd-detail-line"><strong>${key.trim()}:</strong>${value.join(':')}</p>`;
            });
        }
        if (product.agingProcess) extraHTML += `<p class="pd-detail-line"><strong>Aging Process:</strong> ${product.agingProcess}</p>`;
        if (product.foodPairing) extraHTML += `<p class="pd-detail-line"><strong>Food Pairing:</strong> ${product.foodPairing.replace(/\n/g, ' ')}</p>`;
        if (product.serving) {
            product.serving.split('\n').forEach(line => {
                if (line.trim() === '') return;
                const [key, ...value] = line.split(':');
                extraHTML += `<p class="pd-detail-line"><strong>${key.trim()}:</strong>${value.join(':')}</p>`;
            });
        }
        if (product.alcohol) extraHTML += `<p class="pd-detail-line"><strong>Alcohol Content:</strong> ${product.alcohol}% vol.</p>`;
        if (product.volume) extraHTML += `<p class="pd-detail-line"><strong>Volume:</strong> ${product.volume}</p>`;
        extraDetails.innerHTML = extraHTML;

        // Show "Show More" only if the content actually overflows the collapsed height
        const moreEl = document.getElementById('pd-more-details');
        const showMoreBtn = document.getElementById('pd-show-more-btn');
        moreEl.classList.remove('expanded');
        showMoreBtn.classList.remove('expanded');
        showMoreBtn.innerHTML = 'Show More <i class="fas fa-chevron-down"></i>';
        setTimeout(() => {
            showMoreBtn.style.display = moreEl.scrollHeight > moreEl.clientHeight + 10 ? 'block' : 'none';
        }, 450);

        // --- Related Categories ---
        const relatedCategoriesEl = document.getElementById('pd-related-categories');
        let relatedHTML = '';
        if (product.color) relatedHTML += `<div class="pd-related-row"><span class="pd-related-label">Wine Type</span><span class="pd-related-value">${product.color} Wine</span></div>`;
        if (product.grapeVarieties) relatedHTML += `<div class="pd-related-row"><span class="pd-related-label">Grape Varieties</span><span class="pd-related-value">${product.grapeVarieties}</span></div>`;
        if (product.brand) relatedHTML += `<div class="pd-related-row"><span class="pd-related-label">Winery</span><span class="pd-related-value">${product.brand}</span></div>`;
        relatedCategoriesEl.innerHTML = relatedHTML;
        document.querySelector('.pd-related-title').style.display = relatedHTML ? 'block' : 'none';

        // --- Customer Reviews label with count ---
        const reviewsToggleLabel = document.querySelector('#pd-reviews-toggle span');
        reviewsToggleLabel.textContent = product.reviewCount ? `Customer Reviews (${product.reviewCount})` : 'Customer Reviews';
    }
function renderRelatedProducts(products) {
        ...
        products.forEach(prod => {
            ...
            const slidesHTML = imageUrls.map((url, i) => 
                `<div class="slideshow-item ${i===0?'active':''}">
                    <img src="${url}" alt="${prod.name} view ${i + 1}" loading="lazy" width="300" height="400" style="aspect-ratio: 3/4; object-fit: contain;">
                 </div>`
            ).join('');
...
function initContentReveal() {
    ...
    (async () => {
        ...
        const productSlug = getProductSlugFromUrl();
        if (!productSlug) {
            document.querySelector('.pd-section .container').innerHTML = '<h2>Product not found. Please check the URL.</h2>';
            return;
        }
        ...
        try {
            const product = await fetchProductBySlug(productSlug);
            if (!product) {
                document.querySelector('.pd-section .container').innerHTML = `<h2>Product not found.</h2><p>We couldn't find a product with the slug "${productSlug}".</p>`;
                return;
            }
            ...
            document.getElementById('product-gallery-main').addEventListener('click', (e) => { const wishlistBtn = e.target.closest('.wishlist-toggle-btn'); if (wishlistBtn) toggleWishlist(wishlistBtn.dataset.productId); });

            // Buy Now: add to cart with the selected quantity, then go straight to checkout
            document.getElementById('pd-buy-now-btn').addEventListener('click', async () => {
                await addToCart(product.id, parseInt(quantityInput.value));
                window.location.href = `${BASE_PATH}/cart.html`;
            });

            // Compare (placeholder — no comparison feature exists yet)
            document.getElementById('pd-compare-btn').addEventListener('click', () => {
                showToast('Compare is coming soon!', 'info');
            });

            // Wishlist button in the actions row (shares .wishlist-toggle-btn styling/state logic)
            document.getElementById('pd-wishlist-btn').addEventListener('click', () => toggleWishlist(product.id));

            // "Show More" — expand the collapsed Aroma/Aging/Food Pairing block
            document.getElementById('pd-show-more-btn').addEventListener('click', function() {
                const moreEl = document.getElementById('pd-more-details');
                const expanded = moreEl.classList.toggle('expanded');
                this.classList.toggle('expanded', expanded);
                this.innerHTML = expanded ? 'Show Less <i class="fas fa-chevron-down"></i>' : 'Show More <i class="fas fa-chevron-down"></i>';
            });

            // Customer Reviews collapsible panel
            document.getElementById('pd-reviews-toggle').addEventListener('click', function() {
                const panel = document.getElementById('pd-reviews-panel');
                const isOpen = panel.style.display !== 'none';
                panel.style.display = isOpen ? 'none' : 'block';
                this.classList.toggle('expanded', !isOpen);
            });

            // Zoom lightbox for the main product image
            ...
            document.getElementById('pd-zoom-btn').addEventListener('click', () => {
                const activeSlideImg = document.querySelector('#product-gallery-main .slideshow-item.active img');
                if (activeSlideImg) {
            ...
            document.getElementById('pd-lightbox-close').addEventListener('click', () => lightbox.classList.remove('active'));
            ...
            document.querySelector('.pd-section .container').innerHTML = '<h2>Error loading product details. Please try again.</h2>';
        }
    })();
}
